...
class FLSTM(nn.Module):
  def __init__(self,input_dim=14,output_dim=1,rnn_size=200,dense_size=8,dropout=0.1,batch_first=True,bidirectional=False,rnn_layers=2,n_scalars=14,n_profiles=2,profile_size=64,layer_sizes_spatial=[16,8,8], kernel_size_spatial=3, linear_size=5, linear_layer_num=2,wipe_size =0,device=None): 
      super(FLSTM, self).__init__()
      self.n_profiles = n_profiles
      self.pre_rnn_network = InputBlock(n_scalars, n_profiles,profile_size, layer_sizes_spatial, kernel_size_spatial, linear_size, dropout,linear_layer_num,wipe_size = wipe_size)
...
class FTTLSTM(nn.Module):
  def __init__(self,tt_dense=20,cell_steps=10,cell_rank=5,cell_order=1,input_dim=14,output_dim=1,dense_size=8,dropout=0.1,batch_first=True,n_scalars=14,n_profiles=2,profile_size=64,layer_sizes_spatial=[16,8,8], kernel_size_spatial=3,  linear_layer_num=2,wipe_size = 0,device=None): 
...
class FTCN(nn.Module):
    def __init__(self,n_scalars,n_profiles,profile_size,layer_sizes_spatial,
                 ...
                 num_channels_tcn,kernel_size_temporal,dropout=0.1,linear_layer_num=2,wipe_size = 0):
        super(FTCN, self).__init__()
        self.lin = InputBlock(n_scalars, n_profiles,profile_size, layer_sizes_spatial, kernel_size_spatial, linear_size, dropout,linear_layer_num,wipe_size = wipe_size)
...
class InputBlock(nn.Module):
    def __init__(self, n_scalars, n_profiles,profile_size, layer_sizes, kernel_size, linear_size, dropout=0.2,linear_layer_num=2,wipe_size = 44):
...
#             self.downsample.weight.data.normal_(0, 0.01)
    ...
    def forward(self, x):
        if self.n_profiles == 0:
            ...
            x_profiles[:,:-1,:self.wipe_size] = 0.0
